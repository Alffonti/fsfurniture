/*global fiftyshadesfurniture_sticky_add_to_cart_params */
;(function () {
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		if (typeof fiftyshadesfurniture_sticky_add_to_cart_params === 'undefined') {
			return
		}

		const trigger = document.getElementsByClassName(
			fiftyshadesfurniture_sticky_add_to_cart_params.trigger_class
		)
			...
			const stickyAddToCartToggle = function () {
				if (
					...
					stickyAddToCart[0].classList.add(
						'fiftyshadesfurniture-sticky-add-to-cart--slideInDown'
					)
					stickyAddToCart[0].classList.remove(
						'fiftyshadesfurniture-sticky-add-to-cart--slideOutUp'
					)
				} else if (
				...
				if (product) {
					if (
						...
						const selectOptions = document.getElementsByClassName(
							'fiftyshadesfurniture-sticky-add-to-cart__content-button'
						)
